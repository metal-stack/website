...
## Alternative: Gateway API with Envoy Gateway

As an alternative to nginx-ingress, metal-stack supports the [Kubernetes Gateway API](https://kubernetes.io/docs/concepts/services-networking/gateway/) via [Envoy Gateway](https://gateway.envoyproxy.io/).
This is particularly useful when you need to expose TCP-based services (such as the metal-api gRPC endpoint or NSQ) alongside HTTP services.

Gateway API support is added **in addition to** existing Ingress support and is disabled by default in the metal-roles, allowing operators to migrate gradually.
Both can coexist during the transition period since some external dependencies (such as Thanos and MinIO) still rely on Ingress resources.

A working reference implementation using Envoy Gateway on top of [cloud-provider-kind](https://github.com/kubernetes-sigs/cloud-provider-kind) can be found in the [mini-lab repository](https://github.com/metal-stack/mini-lab).
It deploys Envoy Gateway via Helm and creates a `GatewayClass`, `Gateway`, and `EnvoyProxy` resource with HTTP, HTTPS, and TCP listeners.

When using the Gateway API, define a separate DNS domain for Gateway API routes using `metal_control_plane_gateway_dns` and enable the respective route types on the metal roles. For example, in `group_vars/control-plane/metal.yml`:
...
The same `*_httproute_enabled` / `*_tcproute_enabled` variables are available for other services such as `metal_apiserver`, `headscale`, and `zitadel`. See the [metal-roles/control-plane documentation](https://github.com/metal-stack/metal-roles/tree/master/control-plane) for the full list of supported variables.

:::note
When using the Gateway API approach, you typically set `metal_deploy_ingress: false` to avoid deploying the nginx-ingress controller, since the Gateway API handles all traffic routing.
:::
