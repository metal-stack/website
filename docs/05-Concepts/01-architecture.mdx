...
- **[metal-apiserver](https://github.com/metal-stack/metal-apiserver)**
  The API to manage control plane resources like machines, switches, operating system images, machine sizes, networks, IP addresses and more. The exposed API serves gRPC and HTTPS through ConnectRPC. The metal-apiserver stores the state of these entities in a [RethinkDB](https://rethinkdb.com/) database. The metal-apiserver also has its own IP address management ([go-ipam](https://github.com/metal-stack/go-ipam)), which writes IP address and network allocations into a PostgreSQL backend.
- **[tenant-apiserver](https://github.com/metal-stack/tenant-apiserver)**
  Manages tenant and project entities, which can be described as entities used for company-specific resource separation and grouping. Having these "higher level entities" managed by a separate microservice was a design choice that allows to re-use the information by other microservices without having them to know the metal-apiserver at all. The tenant data gets persisted in a dedicated PostgreSQL database.
...
- **[valkey](https://valkey.io/)**
  A key/value datastore (not developed by the metal-stack) used for specific tasks like task queueing and decoupling microservices.
...
export function MachineAllocationYT(){
